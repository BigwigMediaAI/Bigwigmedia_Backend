...
exports.getObjectByLabel = async (req, res) => {
  try {
    ...
    if (req.user) {
      const user = await User.findById(req.user._id);
      const bookmarks = user.bookmarks.map((bookmark) => bookmark.toString());
  ...
  try {
    ...
    for (let i = 0; i < object.groups.length; i++) {
      for (let j = 0; j < object.groups[i].length; j++) {
        prompt += object.groups[i][j].gpt.replace("{%%}", groups[i][j]) + " ";
      }
    }
...
exports.updateObject = async (req, res) => {
  try {
    const object = await Objects.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
